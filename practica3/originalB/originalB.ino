...
#define inputYd 2 // analog input for potentiometer #3 - yd
#define outputYr 6 // analog output for real response of neural network
#define learnButton 3 // digital input for pushButton #1 - learn pattern
#define modeButton 2 // digital input for pushButton #2 - mode
#define ledLearning 9 // digital output for red led - learning pattern process
#define ledTraining 10 // digital output for yellow led - training mode
#define ledRunning 11 // digital output for green led - run mode
#define ledBuiltIn 13 // built-in led for indications of sytem mode (on = autonomous training, off = pc training)

//Definitions for tags in serial communications
#define START 83 //ascii of 'S' will be used as START DATA tag
#define END 69 //ascii of 'E' will be used as END DATA tag
#define SEPARATOR 30 //ascii of RS (register separator) will be used as DATA BYTES SEPARATOR
#define RFPT 84 //ascii of 'T' will be used as ID TAG in the data reception from a computer
...
void serialEvent()
{
    if(Serial.available()==9){
        ...
        if(pcMode==true & inputBuffer[0]==RFPT & inputBuffer[1]==RFPT & inputBuffer[7]==RFPT &
        inputBuffer[8]==RFPT){
...
void nnLearn()
{
    ...
    else
    {
        ...
        //didn't occur maximum sensibility, so we must check if there's any avaliable neuron or it's going to be necesary reuse one.
        if(CN+1 > NTN)
        {
